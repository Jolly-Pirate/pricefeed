...
start() {
	[[ ! -z $(docker ps -a | grep $DOCKER_NAME) ]] && boldtext "Container already started"  ||  ( boldtext "Starting container..." && docker run -h "pricefeed-container" -e TZ=$(cat /etc/timezone) -v $(pwd)/app:/home/pricefeed/app -u `stat -c "%u:%g" app` -itd --name $DOCKER_NAME pricefeed)
}
