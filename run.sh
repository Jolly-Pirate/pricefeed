...
build() {
  ...
  docker build --no-cache -t pricefeed .
}
...
rebuild() {
  ...
  docker build --no-cache -t pricefeed .
  # clean up containers and images remnants
...
start() {
  [[ ! -z $(docker ps -a | grep $DOCKER_NAME) ]] && boldtext "Container already started" || (
    boldtext "Starting container..."
    docker run -itd --restart always -h $DOCKER_NAME -e TZ=$(cat /etc/timezone) -v $(pwd)/app:/home/pricefeed/app -u $(stat -c "%u:%g" app) --name $DOCKER_NAME pricefeed
  )
  logs
}

stop() {
  [[ ! -z $(docker ps -a | grep $DOCKER_NAME) ]] && (
    boldtext "Stopping container..."
    docker kill $DOCKER_NAME
    docker rm -f $DOCKER_NAME
  ) || boldtext "Container not running"
}
